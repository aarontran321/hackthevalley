...
export function BarcodeCamera({ onDetected, onClose }: { onDetected: (code: string) => void; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    return () => { live = false; controls?.stop(); };
  }, [onDetected]);
  return (
    <div style={{ padding: 16, background: "#17221e", borderRadius: 18, position: "relative", color: "white" }}>
      ...
      {error ? <div style={{ minHeight: 220, display: "grid", placeItems: "center", textAlign: "center", padding: 30 }}><div><CameraOff size={30} /><p>{error}</p></div></div> : <video ref={video} style={{ width: "100%", minHeight: 260, maxHeight: 390, objectFit: "cover", borderRadius: 12 }} />}
      <div style={{ textAlign: "center", fontSize: 12, opacity: .8, paddingTop: 10 }}>Hold the barcode steady inside the camera view.</div>
    </div>
  );
}
